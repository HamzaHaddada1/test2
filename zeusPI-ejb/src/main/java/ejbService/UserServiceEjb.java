...
public ArrayList<View> allmyView(User u1) {
	...
	ArrayList<View>	Views = (ArrayList<View>) em.createQuery("FROM  View WHERE Vieweduser="+u1.getId()).getResultList();
...
public ArrayList<Object> serch(String s){
	ArrayList<Object> userserch =(ArrayList<Object>) em.createQuery("FROM User WHERE UPPER( CONCAT(Firstname, ' ', Lastname)) LIKE '%UPPER("+s+")%'").getResultList();
//////////////////////////////////////////////////////////////UPPER(adress) LIKE '%UPPER("+u.getVille()+")%'").getResultList();	
return userserch;
}
